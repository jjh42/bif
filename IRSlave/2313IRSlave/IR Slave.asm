...
;	Mod. Number:	18
;	Mod. Date:		21 August 2001
;
;	Note: Should probably leave Tx always enabled once switch to RS-485!
;			Need to decide on dump and error messages
;
;	Baud rate for comms is 19,200 baud, 7-bits with even parity.
;	No messages end with CR now.
;
;	Accepts the following messages (at 19,200 baud):
;		i			Poll for response (replies j if nothing to send)
;		IV			Poll for version number (replies IVdddd)
;		ILhh		Set LED function to hh
;						'0'	;Always off
...
;						to set hex address aaaa to hex value dd
;
...
;	Pin-15	PB3	(OC1)	Out RS-485 direction control
;	Pin-16	PB4			In	Unused
;	Pin-17	PB5	(MOSI)	In	Unused
;	Pin-18	PB6	(MISO)	In	Unused
;	Pin-19	PB7	(SCK)	In	Unused
;
.EQU	PortBSetup	= 0b00001111
.EQU	LEDPort		= PORTB
	.EQU	FrontLEDPin		= 0
	.EQU	RearLEDPin		= 1
	.EQU	RunningLEDPin	= 2
.EQU	CommsControlPort = PORTB
	.EQU	CommsDirnPin	= 3		;0=Rx, 1=Tx
;
;*****************************************************************************
;
; Port-D:
...
; We only accept "i", "IV", "ILhh", "IDx" amd "IZxaaaadd" messages
...
; Don't check it here, but check for end of three character messages
	lds		ISRTempUb,ComRxBuf+1	;Get the second character
	cpi		ISRTempUb,'D'	;Dump memory message ?
...
; Don't check it here, but check for end of four character messages
	lds		ISRTempUb,ComRxBuf+1	;Get the second character
	cpi		ISRTempUb,'L'	;LED control message ?
...
;
;	Turns off the RS-485 transmitter
;
;*****************************************************************************
...
ISR_UTXC:
	...
	cbi		CommsControlPort,CommsDirnPin	;Turn RS-485 Tx off (set low)
...
; (Deleted because of lack of flash space)
...
SRDLoop2:
	...
	popw	z				;Get the register memory address for this byte
...
; Finished all 1K bytes -- send a final line
;	ldi		ParamReg,'I'
;	rcall	SendTxChar
;	ldi		ParamReg,'D'
;	rcall	SendTxChar
...
;;	out		EEARL,zl
;	out		EEAR,zl			;Output the 7-bit address
...
STCBNFOk:
	...
	sbi		UCR,TXEN		;Enable the transmitter
	sbi		CommsControlPort,CommsDirnPin	;Turn RS-485 Tx on (set high)
	sbi		UCR,UDRIE		;Enable the TX ready (UDRE) interrupt
...
; See if anything has been received for me from the controller
	tst		HaveComRxMsg
	brze	JustReturn	;No, return if no message yet

;*****************************************************************************

; We have an infraRed message -- now which message is it (i, IL, IV, ID, or IZ)?
...
NotComD:
	...
	rjmp	RxInvalidMessage
ValidLMsg:
	mov		LEDFn,yl			;Save the validated function byte
...
NotComL:
	...
	brne	NotComZ
; It's a memory set message	
	ld		TempUb,z+		;Save the third character (Should be E, R or S)
	ld		ParamReg,z+		;Get the fourth character
	rcall	ProcessFirstDigit
	brcs	RxInvalidMessage
	ld		ParamReg,z+		;Get the fifth character
	rcall	ProcessNextDigit
	brcs	RxInvalidMessage
	ld		ParamReg,z+		;Get the sixth character
	rcall	ProcessNextDigit
	brcs	RxInvalidMessage
	ld		ParamReg,z+		;Get the seventh character
	rcall	ProcessNextDigit
	brcs	RxInvalidMessage
	stsw	MemAddress,y	;Save the memory address
	ld		ParamReg,z+		;Get the eighth character
	rcall	ProcessFirstDigit
	brcs	RxInvalidMessage
	ld		ParamReg,z+		;Get the ninth character
	rcall	ProcessNextDigit
	brcs	RxInvalidMessage
	ld		ParamReg,z
	tst		ParamReg		;Error if it's not the final null
...
;NotZE:
	cpi		TempUb,'S'
	brne	NotZS
	st		z,yl		;Write the value to the address
NotZS:
	rjmp	RxInvalidMessage
NotComZ:
